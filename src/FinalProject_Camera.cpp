...
int main(int argc, const char *argv[])
{
    ...
    ofstream csv("../sample.csv",ios_base::app);
    csv <<"detector type"<<","
        ...
        <<"TTC Lidar"<<","
        <<"matched points in currBB"<<","
        <<"TTC Camera"<<","
        <<endl;
    vector<mc> diff_combo;
    const vector<std::string> detectorlist{"HARRIS", "ORB","SHITOMASI", "SIFT","FAST", "BRISK",  "AKAZE"};
    const vector<std::string> descriptorlist{ "FREAK","BRIEF", "AKAZE",  "ORB",  "SIFT" ,"BRISK"};
    ...
    /* MAIN LOOP OVER ALL IMAGES */
        ...
        for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
        {
            ...
            cout<<"lidar points size: " <<(dataBuffer.end() - 1)->lidarPoints.size()<<endl;
            ...
            cout<<"boundingboxes: "<<(dataBuffer.end()-1)->boundingBoxes.size()<<endl;
            ...
            if (dataBuffer.size() > 1) // wait until at least two images have been processed
            {
                ...
                for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                {
                    ...
                    if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                    {
                        ...
                        //cout<<"kptmatches size : "<<currBB->kptMatches.size()<<endl;  
                        csv<<currBB->kptMatches.size()<<",";         
    ...
    }// eof loop for over all combinations
    return 0;
}
